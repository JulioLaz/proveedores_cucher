...
#             Ranking #{info_prov['Ranking']} | ID: {id_proveedor}
#         </p>
#     </div>
...
#     # Obtener IDs del proveedor (incluye IDs unificados)
#     ids_a_buscar = obtener_ids_originales(id_proveedor)
    
#     # Filtrar artículos del proveedor
...
#     if 'costo_total_articulo' not in articulos_prov.columns:
#         if 'margen_porc_all' in articulos_prov.columns:
#             articulos_prov['costo_total_articulo'] = articulos_prov['venta_total_articulo'] * (1 - articulos_prov['margen_porc_all'])
#         else:
#             margen_general = info_prov['Rentabilidad %'] / 100
#             articulos_prov['costo_total_articulo'] = articulos_prov['venta_total_articulo'] * (1 - margen_general)
    
#     # Calcular utilidad por artículo
#     articulos_prov['utilidad_articulo'] = articulos_prov['venta_total_articulo'] - articulos_prov['costo_total_articulo']
    
#     # ═══════════════════════════════════════════════════════════════
#     # MÉTRICAS POR FAMILIA (TOP 3)
#     # ═══════════════════════════════════════════════════════════════
...
#                 # Obtener artículos de la familia para tooltip
#                 arts_familia = articulos_prov[articulos_prov['familia'] == fam['familia']]
#                 top_arts = arts_familia.nlargest(3, 'venta_total_articulo')
...
#                     df_arts_fam = arts_familia_completos[['idarticulo', 'descripcion', 'venta_total_articulo', 'utilidad_articulo']].copy()
#                     df_arts_fam.columns = ['Código', 'Descripción', 'Venta', 'Utilidad']
#                     df_arts_fam['Ranking'] = range(1, len(df_arts_fam) + 1)
#                     df_arts_fam = df_arts_fam[['Ranking', 'Código', 'Descripción', 'Venta', 'Utilidad']]
...
#                 top_arts_sub = arts_subfam.nlargest(3, 'venta_total_articulo')
...
#                     # Crear DataFrame para mostrar
#                     df_arts_subfam = arts_subfam_completos[['idarticulo', 'descripcion', 'venta_total_articulo', 'utilidad_articulo']].copy()
#                     df_arts_subfam.columns = ['Código', 'Descripción', 'Venta', 'Utilidad']
#                     df_arts_subfam['Ranking'] = range(1, len(df_arts_subfam) + 1)
#                     df_arts_subfam = df_arts_subfam[['Ranking', 'Código', 'Descripción', 'Venta', 'Utilidad']]
...
#     if art_sin_stock > 0 or art_con_exceso > 0:
...
#                             # Crear DataFrame para mostrar
#                             df_sin_stock = arts_sin_stock[['idarticulo', 'descripcion', 'familia', 'subfamilia', 'venta_total_articulo', 'utilidad_articulo']].copy()
#                             df_sin_stock.columns = ['Código', 'Descripción', 'Familia', 'Subfamilia', 'Venta', 'Utilidad']
#                             df_sin_stock['Ranking'] = range(1, len(df_sin_stock) + 1)
#                             df_sin_stock = df_sin_stock[['Ranking', 'Código', 'Descripción', 'Familia', 'Subfamilia', 'Venta', 'Utilidad']]
...
#                             # Ordenar por costo de exceso descendente
#                             arts_con_exceso = arts_con_exceso.sort_values('costo_exceso_STK', ascending=False)
                            
#                             # Crear DataFrame para mostrar
#                             df_con_exceso = arts_con_exceso[[
#                                 'idarticulo', 'descripcion', 'familia', 'subfamilia', 
#                                 'exceso_STK', 'costo_exceso_STK', 'STK_TOTAL'
#                             ]].copy()
#                             df_con_exceso.columns = ['Código', 'Descripción', 'Familia', 'Subfamilia', 'Exceso (unid.)', 'Costo Exceso', 'Stock Total']
#                             df_con_exceso['Ranking'] = range(1, len(df_con_exceso) + 1)
#                             df_con_exceso = df_con_exceso[['Ranking', 'Código', 'Descripción', 'Familia', 'Subfamilia', 'Stock Total', 'Exceso (unid.)', 'Costo Exceso']]
...
def mostrar_panel_proveedor(proveedor_seleccionado, id_proveedor, info_prov, 
    ...
    # ═══════════════════════════════════════════════════════════════
    # OBTENER COSTOS Y UTILIDAD REALES DEL PERÍODO
    # ═══════════════════════════════════════════════════════════════
    
    # PRIORIDAD 1: Usar utilidad_total si ya existe (viene calculada correctamente)
    if 'utilidad_total' in articulos_prov.columns:
        articulos_prov['utilidad_articulo'] = articulos_prov['utilidad_total']
    elif 'utilidad_articulo' in articulos_prov.columns:
        # Ya existe con este nombre
        pass
    else:
        # PRIORIDAD 2: Calcular desde costo_total
        if 'costo_total_articulo' in articulos_prov.columns:
            articulos_prov['utilidad_articulo'] = articulos_prov['venta_total_articulo'] - articulos_prov['costo_total_articulo']
        elif 'costo_total' in articulos_prov.columns:
            articulos_prov['utilidad_articulo'] = articulos_prov['venta_total_articulo'] - articulos_prov['costo_total']
        else:
            # PRIORIDAD 3: Calcular desde cnt_corregida * costo_unit
            if 'cnt_corregida' in articulos_prov.columns and 'costo_unit' in articulos_prov.columns:
                articulos_prov['costo_total_articulo'] = articulos_prov['cnt_corregida'] * articulos_prov['costo_unit']
                articulos_prov['utilidad_articulo'] = articulos_prov['venta_total_articulo'] - articulos_prov['costo_total_articulo']
            elif 'cantidad_vendida' in articulos_prov.columns and 'costo_unit' in articulos_prov.columns:
                articulos_prov['costo_total_articulo'] = articulos_prov['cantidad_vendida'] * articulos_prov['costo_unit']
                articulos_prov['utilidad_articulo'] = articulos_prov['venta_total_articulo'] - articulos_prov['costo_total_articulo']
            else:
                # Último recurso: usar margen general del proveedor
                margen_general = info_prov['Rentabilidad %'] / 100
                articulos_prov['costo_total_articulo'] = articulos_prov['venta_total_articulo'] * (1 - margen_general)
                articulos_prov['utilidad_articulo'] = articulos_prov['venta_total_articulo'] - articulos_prov['costo_total_articulo']
    
    # Calcular margen % real por artículo
    ...
    # ═══════════════════════════════════════════════════════════════
    # VALIDACIÓN: Comparar con datos del ranking
    # ═══════════════════════════════════════════════════════════════
    
    venta_total_articulos = articulos_prov['venta_total_articulo'].sum()
    utilidad_total_articulos = articulos_prov['utilidad_articulo'].sum()
    margen_calculado = (utilidad_total_articulos / venta_total_articulos * 100) if venta_total_articulos > 0 else 0
    
    # Verificar artículos sin familia/subfamilia
    ...
    # Mostrar alerta si hay inconsistencia significativa
    diferencia_margen = abs(margen_calculado - info_prov['Rentabilidad %'])
    
    if diferencia_margen > 2 or len(arts_sin_familia) > 0:
        st.warning(f"""
        ⚠️ **Nota sobre los márgenes mostrados:**
        
        - **Margen del Ranking**: {info_prov['Rentabilidad %']:.1f}% (fuente: datos agregados del sistema)
        - **Margen Calculado**: {margen_calculado:.1f}% (fuente: suma de artículos individuales)
        - **Diferencia**: {diferencia_margen:.1f} puntos porcentuales
        
        {'📋 ' + str(len(arts_sin_familia)) + ' artículos sin familia asignada' if len(arts_sin_familia) > 0 else ''}
        {'📋 ' + str(len(arts_sin_subfamilia)) + ' artículos sin subfamilia asignada' if len(arts_sin_subfamilia) > 0 else ''}
        
        **Posibles causas de la diferencia:**
        - Artículos sin clasificación (familia/subfamilia vacía)
        - Diferentes fuentes de cálculo de costos
        - Productos con márgenes extremos (muy negativos o muy positivos)
        
        👉 **Recomendación**: Verifica la clasificación de productos en el sistema
        """)
