...
def show_global_dashboard(df_proveedores, query_function, credentials_path, project_id, bigquery_table):
    ...
    with container:
        ...
        with col_fam1:
            # Obtener familias únicas del dataframe original
            familias_disponibles = ["Todas"]
            if 'familia' in df_proveedores.columns:
                familias_unicas = sorted([f for f in df_proveedores['familia'].dropna().unique() if str(f).strip()])
                familias_disponibles.extend(familias_unicas)
        ...
        with col_fam2:
            # Filtrar subfamilias según la familia seleccionada
            subfamilias_disponibles = ["Todas"]
            
            if 'subfamilia' in df_proveedores.columns:
                if familia_seleccionada == "Todas":
                    subfamilias_unicas = sorted([s for s in df_proveedores['subfamilia'].dropna().unique() if str(s).strip()])
                else:
                    df_familia_temp = df_proveedores[df_proveedores['familia'] == familia_seleccionada]
                    subfamilias_unicas = sorted([s for s in df_familia_temp['subfamilia'].dropna().unique() if str(s).strip()])
                
                subfamilias_disponibles.extend(subfamilias_unicas)
        ...
        with col_fam3:
            # Aplicar filtros temporalmente para contar productos
            df_temp_filtrado = df_proveedores.copy()
            
            if familia_seleccionada != "Todas" and 'familia' in df_proveedores.columns:
                df_temp_filtrado = df_temp_filtrado[df_temp_filtrado['familia'] == familia_seleccionada]
            
            if subfamilia_seleccionada != "Todas" and 'subfamilia' in df_proveedores.columns:
                df_temp_filtrado = df_temp_filtrado[df_temp_filtrado['subfamilia'] == subfamilia_seleccionada]
            
            st.metric(
                "🎯 Artículos", 
                f"{len(df_temp_filtrado):,}", 
                border=True,
                help="Artículos en el filtro actual"
            )
    ...
    if ranking is None or ranking.empty:
        ...
        print(f"\n⚠️  No se encontraron datos para los filtros aplicados")
        return
    
    print(f"\n{'─'*80}")
    print(f"📊 DATOS PROCESADOS:")
    print(f"   🏢 Proveedores: {len(ranking)}")
    print(f"   📦 Artículos únicos en ventas: {df_ventas['idarticulo'].nunique():,}")
    ...
    nombre_archivo = f"ranking_proveedores_{datetime.now().strftime('%d%B%Y')}"
    if familia_seleccionada != "Todas":
        nombre_archivo += f"_FAM-{familia_seleccionada.replace(' ', '-')}"
    if subfamilia_seleccionada != "Todas":
        nombre_archivo += f"_SUBFAM-{subfamilia_seleccionada.replace(' ', '-')}"
    nombre_archivo += ".xlsx"
    ...
    print(f"\n{'='*80}")
    print(f"✅ DASHBOARD COMPLETADO EN {tiempo_total:.2f}s")
    print(f"   📊 Filtros aplicados: Familia={familia_seleccionada}, Subfamilia={subfamilia_seleccionada}")
    print(f"   🏢 Proveedores analizados: {len(ranking)}")
    print(f"   💰 Venta total: ${ranking['Venta Total'].sum():,.0f}")
    print(f"{'='*80}\n")