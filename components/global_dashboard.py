...
def show_global_dashboard(df_proveedores, query_function, credentials_path, project_id, bigquery_table):
    ...
    with container:
        ...
        with col1:
            ...
            periodo_seleccionado = st.selectbox(
                ...
                options=list(periodo_opciones.keys()),
                index=0,
                key='periodo_selector'  # ✅ Key única para evitar conflictos
            )
    ...
    print(f"\n📅 Período seleccionado: {periodo_seleccionado}")
    print(f"   └─ Desde: {fecha_desde} | Hasta: {fecha_hasta} | Días: {dias_periodo}")
    ...
    # Remover el sistema manual de cache_key, confiar en el caché de Streamlit
    
    with st.spinner(f"🔄 Cargando datos ({dias_periodo} días)..."):
        inicio_carga = time.time()
        
        # Las funciones ya tienen @st.cache_data, se actualizarán automáticamente
        # cuando cambien los parámetros (fecha_desde, fecha_hasta)
        df_ventas = get_ventas_data(
            credentials_path, 
            project_id, 
            bigquery_table,
