...
import plotly.graph_objects as go  # ← AGREGAR ESTA LÍNEA
...
def clasificar_sucursal(sucursal):
    """Clasifica sucursal en Chaco o Corrientes"""
    sucursal_str = str(sucursal).strip().lower()
    
    if sucursal_str in ['express', 'formosa', 'tirol', 'hiper', '6', '7', '8']:
        return 'chaco'
    elif sucursal_str in ['corrientes', 'central', '1', '3', '4', '5']:
        return 'corrientes'
    else:
        return 'otra'
...
def generar_columnas_demanda(ventas_totales, ventas_por_region, cantidad_semanas):
    """Genera las columnas de demanda según el parámetro cantidad_semanas"""
    # Siempre 1 semana
    ...
    # Si cantidad_semanas > 2, calcular adicional
    if cantidad_semanas > 2:
        df_n_semanas = calcular_demanda_potencial_bloques(ventas_totales, n_semanas=cantidad_semanas)
        df_n_semanas = df_n_semanas[['idartalfa', f'demanda_potencial_{cantidad_semanas}_bloques']]
        df_n_semanas.rename(columns={
            f'demanda_potencial_{cantidad_semanas}_bloques': f'demanda_potencial_proximas_{cantidad_semanas}_semanas'
        }, inplace=True)
        
        df_resultado = df_resultado.merge(df_n_semanas, on='idartalfa', how='left')
    
    df_distribucion = calcular_demanda_por_region(ventas_por_region, df_resultado, n_semanas=cantidad_semanas)
...
def generar_columnas_ventas_bloques(df_con_bloques, df_final, cantidad_bloques=4):
    ...
    if len(nombres_bloques) >= 4:
        fecha_inicio = nombres_bloques[0].split('_')[1]  # Ejemplo: 01dec
        fecha_fin = nombres_bloques[-1].split('_')[-1]  # Ejemplo: 28dec
...
def calcular_presupuesto(df_final, cantidad_semanas):
    """Calcula el presupuesto basado en demanda y stock"""
    # 1 SEMANA
...
def crear_libro_excel_formateado(df_final, cantidad_semanas, nombres_bloques, proveedor_nombre):
    ...
    df_chaco = df_final[(df_final['chaco_abastecer_1sem'] > 0) | (df_final['chaco_abastecer_2sem'] > 0)].copy()
    ...
    columnas_chaco = ['idartalfa', 'descripcion', 'uxb', 'ultimos_7_dias', 'ultimos_14_dias', 
                      'chaco_abastecer_1sem', 'chaco_abastecer_2sem',
                      'costo_unitario', 'presupuesto_chaco_1sem', 'presupuesto_chaco_2sem']
    ...
    df_corr = df_final[(df_final['corr_abastecer_1sem'] > 0) | (df_final['corr_abastecer_2sem'] > 0)].copy()
    ...
    columnas_corr = ['idartalfa', 'descripcion', 'uxb', 'ultimos_7_dias', 'ultimos_14_dias',
                     'corr_abastecer_1sem', 'corr_abastecer_2sem',
                     'costo_unitario', 'presupuesto_corrientes_1sem', 'presupuesto_corrientes_2sem']
...
def render_tab_prediccion_presupuesto(df_proveedores, config):
    ...
    st.markdown("""
        ...
        margin-bottom: 2rem;">
            ...
            <p style="color: white; margin-top: 0.5rem; font-size: 1.2rem;">
                Sistema de bloques de 7 días con proyección flexible
            </p>
        </div>
   ...
   #          options=proveedores_disponibles['proveedor'].tolist(),
   #          index=0  # SALTA REFRESCOS por defecto (ya que está primero alfabéticamente)
   #      )
   ...
   #          "📅 Semanas a pronosticar:",
   #          min_value=1,
   #          max_value=4,
   #          value=1,
   #          help="Cantidad de semanas hacia adelante para la predicción"
   #      )
   # ═══════════════════════════════════════════════════════════════════════════
   # SELECTOR DE PROVEEDOR
   # ═══════════════════════════════════════════════════════════════════════════
    ...
    with col1:
      ...
      proveedor_seleccionado = st.selectbox(
         ...
         index=index_salta  # ← SALTA REFRESCOS por defecto
      )
    ...
    with col2:
      semanas_pronostico = st.radio(
         "📅 Semanas a pronosticar:",
         options=[1, 2, 3, 4],
         index=0,  # 1 semana por defecto
         horizontal=True,
         help="Cantidad de semanas hacia adelante para la predicción"
      )    
    ...
    if id_proveedor_seleccionado == SALTA_REFRESCOS_ID:
        ...
        # Filtrar artículos del proveedor desde df_proveedores
    ...
    if st.button("🚀 Generar Análisis y Presupuesto", type="primary", width='stretch'):
        ...
        with st.spinner("Procesando datos..."):
            ...
            # Generar columnas de demanda
            df_demanda = generar_columnas_demanda(ventas_totales, ventas_por_region, semanas_pronostico)
            ...
            df_final = calcular_presupuesto(df_final, semanas_pronostico)
            ...
            nombre_archivo = f"{proveedor_seleccionado.replace(' ', '_')}_presupuesto_{semanas_pronostico}sem_{fecha_str}.xlsx"
            ...
            with col_t1:
                with st.expander("🟧 Ver Tabla CHACO", expanded=False):
                    df_chaco_tabla = df_final[(df_final['chaco_abastecer_1sem'] > 0) | (df_final['chaco_abastecer_2sem'] > 0)].copy()
                    st.dataframe(df_chaco_tabla, width='stretch', height=400)
            
            with col_t2:
                with st.expander("🟦 Ver Tabla CORRIENTES", expanded=False):
                    df_corr_tabla = df_final[(df_final['corr_abastecer_1sem'] > 0) | (df_final['corr_abastecer_2sem'] > 0)].copy()
            ...
            # def calcular_dias_cobertura(row, columna_venta_promedio_diaria):
            #     """Calcula días de cobertura basado en venta promedio diaria"""
            #     if columna_venta_promedio_diaria in row and row[columna_venta_promedio_diaria] > 0:
            #         return row['STK_TOTAL'] / (row[columna_venta_promedio_diaria] / 7)  # dividir entre 7 para venta diaria
            #     return 0
            ...
            def calcular_dias_cobertura(row, columna_venta_periodo, dias_periodo=28):
                ...
                Args:
                    ...
                    columna_venta_periodo: Columna con ventas totales del período (ej: TOTAL_02dec_29dec)
                    dias_periodo: Días del período analizado (default: 28 días = 4 semanas)
                """
            ...
            if columna_total:
               #  import plotly.graph_objects as go
                ...
                df_top_periodo = df_top_periodo.iloc[::-1]  # Invertir para mayor arriba
                
                # Crear label para artículos
                ...
                # Calcular días de cobertura para estos artículos
                df_top_periodo['dias_cobertura'] = df_top_periodo.apply(
                    lambda row: calcular_dias_cobertura(row, columna_total), axis=1
                )
                ...
                with col_g1:
                    st.markdown(f"#### 📦 Top 20 Artículos por Cantidad ({columna_total.replace('TOTAL_', '').replace('_', ' al ')})")
                ...
                with col_g2:
                    ...
                    fig_cobertura.add_trace(go.Bar(
                        ...
                        cliponaxis=False,
                        marker=dict(
                            color=df_top_periodo['color_cobertura'],
                            line=dict(width=0)
                        ),
                        hovertemplate='%{customdata}<extra></extra>',
            ...
            # ═══════════════════════════════════════════════════════════════════
            # GRÁFICOS 2: PRESUPUESTO 1 SEMANA + COBERTURA
            # ═══════════════════════════════════════════════════════════════════
            
            st.markdown("---")
            st.subheader("💰 Análisis de Presupuesto 1 Semana")
            
            # Top 20 por presupuesto_total_1sem
            df_top_presupuesto = df_final.nlargest(20, 'presupuesto_total_1sem').copy()
            df_top_presupuesto = df_top_presupuesto.iloc[::-1]
            
            df_top_presupuesto['articulo_label'] = df_top_presupuesto.apply(
                lambda row: f"{row['idartalfa']} - {row['descripcion'][:30]}..." 
                if len(str(row['descripcion'])) > 30 
                else f"{row['idartalfa']} - {row['descripcion']}", 
                axis=1
            ...
            # Calcular días de cobertura (usando demanda_potencial_proxima_semana)
            df_top_presupuesto['dias_cobertura'] = df_top_presupuesto.apply(
                lambda row: (row['STK_TOTAL'] / (row['demanda_potencial_proxima_semana'] / 7)) 
                if row['demanda_potencial_proxima_semana'] > 0 else 0,
                axis=1
            )
            df_top_presupuesto['cobertura_visual'] = df_top_presupuesto['dias_cobertura'].apply(lambda x: min(x, 31))
            df_top_presupuesto['cobertura_texto'] = df_top_presupuesto['dias_cobertura'].apply(
                lambda x: f"{x:.0f}d" if x <= 31 else f"31d+"
            )
            df_top_presupuesto['color_cobertura'] = df_top_presupuesto['dias_cobertura'].apply(get_color_cobertura)
            
            col_g3, col_g4 = st.columns(2)
            
            with col_g3:
                st.markdown("#### 💵 Top 20 Artículos por Presupuesto 1 Semana")
                
                df_top_presupuesto['Presupuesto_M'] = df_top_presupuesto['presupuesto_total_1sem'] / 1_000_000
                df_top_presupuesto['Texto'] = df_top_presupuesto['presupuesto_total_1sem'].apply(
                    lambda x: f"${x:,.0f}".replace(",", ".")
                )
                
                hover_text_pres = []
                for idx, row in df_top_presupuesto.iterrows():
                    texto = f"<b>{row['articulo_label']}</b><br>"
                    texto += f"Presupuesto 1sem: ${row['presupuesto_total_1sem']:,.2f}<br>"
                    texto += f"Chaco: {int(row['chaco_abastecer_1sem']):,} uds<br>"
                    texto += f"Corrientes: {int(row['corr_abastecer_1sem']):,} uds<br>"
                    texto += f"Stock Total: {int(row['STK_TOTAL']):,}"
                    hover_text_pres.append(texto)
                
                fig_presupuesto = go.Figure()
                
                fig_presupuesto.add_trace(go.Bar(
                    y=df_top_presupuesto['articulo_label'],
                    x=df_top_presupuesto['Presupuesto_M'],
                    orientation='h',
                    text=df_top_presupuesto['Texto'],
                    textposition='outside',
                    cliponaxis=False,
                    marker=dict(
                        color='#27ae60',
                ...
                max_pres = df_top_presupuesto['Presupuesto_M'].max()
            ...
            with col_g4:
                ...
                for idx, row in df_top_presupuesto.iterrows():
                    ...
                    texto += f"Cobertura: {row['dias_cobertura']:.0f} días<br>"
                    texto += f"Stock Total: {int(row['STK_TOTAL']):,}<br>"
                    texto += f"Demanda 1sem: {int(row['demanda_potencial_proxima_semana']):,}"
                ...
                fig_cob2.add_trace(go.Bar(
                    y=df_top_presupuesto['articulo_label'],
                    x=df_top_presupuesto['cobertura_visual'],
                    orientation='h',
                    text=df_top_presupuesto['cobertura_texto'],
                    textposition='outside',
                    cliponaxis=False,
                    marker=dict(
                        color=df_top_presupuesto['color_cobertura'],
                        line=dict(width=0)
                    ),
                    hovertemplate='%{customdata}<extra></extra>',
                ...
                for dia, color in zip([7, 14, 21, 28], ['#e74c3c', '#e67e22', '#f39c12', '#3498db']):
                    fig_cob2.add_vline(
                        x=dia,
                        line_dash="dash",
                        line_color=color,
