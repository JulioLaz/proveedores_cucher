...
def render_tab_prediccion_presupuesto(df_proveedores, config):
    ...
    incluir_formosa = st.checkbox(
        "☑️ Incluir sucursal Formosa en análisis de ventas",
        value=False,
    ...
    if st.button("🚀 Generar Análisis y Presupuesto", type="primary", width='stretch'):
        ...
        with st.spinner("Procesando datos..."):
            ...
            def get_color_cobertura_actual(dias):
                """
                Asigna color a barra de cobertura actual (lo que TENGO)
                Verde = bueno (≥7d), amarillo/naranja = cuidado, rojo = crítico
                """
                if dias >= 7:
                    return '#27ae60'  # Verde
                elif dias >= 3:
            ...
            def get_color_faltante(dias_faltantes):
                """
                Asigna color a barra faltante (lo que NECESITO)
                Rojo = mucho faltante (crítico), amarillo = poco faltante
                """
                if dias_faltantes >= 5:
                    return '#e74c3c'  # Rojo fuerte
                elif dias_faltantes >= 3:
                    return '#e67e22'  # Naranja
                elif dias_faltantes > 0:
                    return '#f39c12'  # Amarillo
                else:
                    return '#27ae60'  # Verde (no falta nada)
            ...
            if columna_total:
                ...
                with col_g1:
                    st.markdown(f"#### 📦 Top 20 por Cantidad ({columna_total.replace('TOTAL_', '').replace('_', ' al ')})")
                ...
                with col_g2:
                    st.markdown("#### ⏱️ Días de Cobertura (Cap: 7 días + Faltante)")
                    
                    # Calcular días faltantes y unidades
                    df_top_periodo['dias_faltantes'] = df_top_periodo['dias_cobertura'].apply(lambda x: max(0, 7 - x))
                    df_top_periodo['unidades_faltantes'] = df_top_periodo.apply(
                        lambda row: int(row['dias_faltantes'] * (row[columna_total] / 28)),
                        axis=1
                    )
                    
                    # Limitar cobertura visual a 7 días
                    df_top_periodo['cobertura_visual_7'] = df_top_periodo['dias_cobertura'].apply(lambda x: min(x, 7))
                    
                    # Textos y colores con sistema de urgencia INVERTIDO
                    df_top_periodo['texto_cobertura'] = df_top_periodo['dias_cobertura'].apply(lambda x: f"{x:.1f}d")
                    df_top_periodo['texto_faltante'] = df_top_periodo['unidades_faltantes'].apply(
                        lambda x: f"{x:,} uds" if x > 0 else ""
                    )
                    df_top_periodo['color_actual'] = df_top_periodo['dias_cobertura'].apply(get_color_cobertura_actual)
                    df_top_periodo['color_faltante'] = df_top_periodo['dias_faltantes'].apply(get_color_faltante)
                    ...
                    for idx, row in df_top_periodo.iterrows():
                        # Hover cobertura actual
                        texto1 = f"<b>{row['articulo_label']}</b><br>"
                        texto1 += f"Cobertura actual: {row['dias_cobertura']:.1f} días<br>"
                        texto1 += f"Stock Total: {int(row['STK_TOTAL']):,}<br>"
                        texto1 += f"Venta Período: {int(row[columna_total]):,}"
                        hover_text_cobertura.append(texto1)
                        
                        # Hover faltante
                        if row['dias_faltantes'] > 0:
                            texto2 = f"<b>{row['articulo_label']}</b><br>"
                            texto2 += f"Días faltantes: {row['dias_faltantes']:.1f}<br>"
                            texto2 += f"Unidades faltantes: {int(row['unidades_faltantes']):,}"
                            hover_text_faltante.append(texto2)
                        else:
                            hover_text_faltante.append("")
                    
                    fig_cobertura = go.Figure()
                    
                    # Trace 1: Cobertura actual (CON TEXTO y colores por urgencia)
                    fig_cobertura.add_trace(go.Bar(
                        y=df_top_periodo['articulo_label'],
                        x=df_top_periodo['cobertura_visual_7'],
                        orientation='h',
                        text=df_top_periodo['texto_cobertura'],
                        textposition='inside',
                        textfont=dict(color='white', size=10, family='Arial Black'),
                        marker=dict(color=df_top_periodo['color_actual'], line=dict(width=0)),
                        hovertemplate='%{customdata}<extra></extra>',
                    ...
                    # Trace 2: Faltante hasta 7 días (colores por urgencia)
                    fig_cobertura.add_trace(go.Bar(
                        y=df_top_periodo['articulo_label'],
                        x=df_top_periodo['dias_faltantes'],
                        orientation='h',
                        text=df_top_periodo['texto_faltante'],
                        textposition='outside',
                        cliponaxis=False,
                        marker=dict(color=df_top_periodo['color_faltante'], line=dict(width=0)),
                        hovertemplate='%{customdata}<extra></extra>',
                        customdata=hover_text_faltante,
                        name='Faltante hasta 7d',
                        showlegend=False
                    ))
                    
                    # Líneas de referencia
                    lineas_dias = [7, 14, 21, 28]
                    colores_lineas = ['#2ecc71', '#e67e22', '#f39c12', '#3498db']
                    ...
                    fig_cobertura.update_layout(
                        height=max(400, 20 * 25),
                        margin=dict(t=20, b=5, l=30, r=120),
                        xaxis=dict(
                            visible=True, range=[0, 33],
            ...
            with col_g3:
                st.markdown("#### 💵 Top 20 por Presupuesto CHACO")
            ...
            with col_g4:
                ...
                # Calcular días faltantes y unidades (USAR chaco_abastecer_1sem directamente)
                df_top_chaco['dias_faltantes'] = df_top_chaco['dias_cobertura'].apply(lambda x: max(0, 7 - x))
                df_top_chaco['unidades_faltantes'] = df_top_chaco['chaco_abastecer_1sem']
                
                # Limitar cobertura visual a 7 días
                df_top_chaco['cobertura_visual_7'] = df_top_chaco['dias_cobertura'].apply(lambda x: min(x, 7))
                
                # Textos y colores con sistema de urgencia INVERTIDO
                df_top_chaco['texto_cobertura'] = df_top_chaco['dias_cobertura'].apply(lambda x: f"{x:.1f}d")
                df_top_chaco['texto_faltante'] = df_top_chaco['unidades_faltantes'].apply(
                    lambda x: f"{int(x):,} uds" if x > 0 else ""
                )
                df_top_chaco['color_actual'] = df_top_chaco['dias_cobertura'].apply(get_color_cobertura_actual)
                df_top_chaco['color_faltante'] = df_top_chaco['dias_faltantes'].apply(get_color_faltante)
                
                # Hover texts
                hover_text_cob2 = []
                hover_text_faltante2 = []
                
                for idx, row in df_top_chaco.iterrows():
                    # Hover cobertura actual
                    texto1 = f"<b>{row['articulo_label']}</b><br>"
                    texto1 += f"Cobertura actual: {row['dias_cobertura']:.1f} días<br>"
                    texto1 += f"Stock CHACO: {int(row['STK_CHACO']):,}<br>"
                    texto1 += f"Demanda Chaco 1sem: {int(row['chaco_cantidad']):,}"
                    hover_text_cob2.append(texto1)
                    ...
                    if row['dias_faltantes'] > 0:
                        ...
                        texto2 += f"Días faltantes: {row['dias_faltantes']:.1f}<br>"
                        texto2 += f"Unidades faltantes: {int(row['unidades_faltantes']):,}"
                        hover_text_faltante2.append(texto2)
                    else:
                        hover_text_faltante2.append("")
                ...
                fig_cob2.add_trace(go.Bar(
                    y=df_top_chaco['articulo_label'],
                    x=df_top_chaco['cobertura_visual_7'],
                    orientation='h',
                    text=df_top_chaco['texto_cobertura'],
                    textposition='inside',
                    textfont=dict(color='white', size=10, family='Arial Black'),
                    marker=dict(color=df_top_chaco['color_actual'], line=dict(width=0)),
                    hovertemplate='%{customdata}<extra></extra>',
                ...
                fig_cob2.add_trace(go.Bar(
                    y=df_top_chaco['articulo_label'],
                    x=df_top_chaco['dias_faltantes'],
                    orientation='h',
                    text=df_top_chaco['texto_faltante'],
                    textposition='outside',
                    cliponaxis=False,
                    marker=dict(color=df_top_chaco['color_faltante'], line=dict(width=0)),
                    hovertemplate='%{customdata}<extra></extra>',
                    customdata=hover_text_faltante2,
                    name='Faltante hasta 7d',
                    showlegend=False
                ))
                
                # Líneas de referencia
                for dia, color in zip([7, 14, 21, 28], ['#2ecc71', '#e67e22', '#f39c12', '#3498db']):
                    fig_cob2.add_vline(
                        x=dia, line_dash="dash", line_color=color, line_width=1.5, opacity=0.6,
                        annotation_text=f"{dia}d", annotation_position="top",
                        annotation_font_size=9, annotation_font_color=color
            ...
            with col_g5:
                st.markdown("#### 💵 Top 20 por Presupuesto CORRIENTES")
            ...
            with col_g6:
                ...
                # Calcular días faltantes y unidades (USAR corr_abastecer_1sem directamente)
                df_top_corr['dias_faltantes'] = df_top_corr['dias_cobertura'].apply(lambda x: max(0, 7 - x))
                df_top_corr['unidades_faltantes'] = df_top_corr['corr_abastecer_1sem']
                
                # Limitar cobertura visual a 7 días
                df_top_corr['cobertura_visual_7'] = df_top_corr['dias_cobertura'].apply(lambda x: min(x, 7))
                
                # Textos y colores con sistema de urgencia INVERTIDO
                df_top_corr['texto_cobertura'] = df_top_corr['dias_cobertura'].apply(lambda x: f"{x:.1f}d")
                df_top_corr['texto_faltante'] = df_top_corr['unidades_faltantes'].apply(
                    lambda x: f"{int(x):,} uds" if x > 0 else ""
                )
                df_top_corr['color_actual'] = df_top_corr['dias_cobertura'].apply(get_color_cobertura_actual)
                df_top_corr['color_faltante'] = df_top_corr['dias_faltantes'].apply(get_color_faltante)
                
                # Hover texts
                hover_text_cob3 = []
                hover_text_faltante3 = []
                
                for idx, row in df_top_corr.iterrows():
                    # Hover cobertura actual
                    texto1 = f"<b>{row['articulo_label']}</b><br>"
                    texto1 += f"Cobertura actual: {row['dias_cobertura']:.1f} días<br>"
                    texto1 += f"Stock Corrientes: {int(row['stk_corrientes']):,}<br>"
                    texto1 += f"Demanda Corrientes 1sem: {int(row['corr_cantidad']):,}"
                    hover_text_cob3.append(texto1)
                    ...
                    if row['dias_faltantes'] > 0:
                        ...
                        texto2 += f"Días faltantes: {row['dias_faltantes']:.1f}<br>"
                        texto2 += f"Unidades faltantes: {int(row['unidades_faltantes']):,}"
                        hover_text_faltante3.append(texto2)
                    else:
                        hover_text_faltante3.append("")
                ...
                fig_cob3.add_trace(go.Bar(
                    y=df_top_corr['articulo_label'],
                    x=df_top_corr['cobertura_visual_7'],
                    orientation='h',
                    text=df_top_corr['texto_cobertura'],
                    textposition='inside',
                    textfont=dict(color='white', size=10, family='Arial Black'),
                    marker=dict(color=df_top_corr['color_actual'], line=dict(width=0)),
                    hovertemplate='%{customdata}<extra></extra>',
                ...
                fig_cob3.add_trace(go.Bar(
                    y=df_top_corr['articulo_label'],
                    x=df_top_corr['dias_faltantes'],
                    orientation='h',
                    text=df_top_corr['texto_faltante'],
                    textposition='outside',
                    cliponaxis=False,
                    marker=dict(color=df_top_corr['color_faltante'], line=dict(width=0)),
                    hovertemplate='%{customdata}<extra></extra>',
                    customdata=hover_text_faltante3,
                    name='Faltante hasta 7d',
                    showlegend=False
                ))
